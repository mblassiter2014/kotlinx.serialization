...
public class ProtoBuf(
    ...
    private open inner class ProtobufDecoder(@JvmField val reader: ProtobufReader) : TaggedDecoder<ProtoDesc>() {
        ...
        override fun beginStructure(descriptor: SerialDescriptor, vararg typeParams: KSerializer<*>): CompositeDecoder =
            when (descriptor.kind) {
                StructureKind.LIST -> RepeatedDecoder(reader, currentTag)
                StructureKind.CLASS, StructureKind.OBJECT, is PolymorphicKind ->
                    ProtobufDecoder(makeDelimited(reader, currentTagOrNull))
                StructureKind.MAP -> MapEntryReader(makeDelimited(reader, currentTagOrNull), currentTagOrNull)
                else -> throw SerializationException("Primitives are not supported at top-level")
            }
        ...
        override fun decodeTaggedEnum(tag: ProtoDesc, enumDescription: SerialDescriptor): Int =
            findIndexByTag(enumDescription, reader.nextInt(ProtoNumberType.DEFAULT), zeroBasedDefault = true)
        ...
        override fun <T> decodeSerializableValue(deserializer: DeserializationStrategy<T>): T = when {
            ...
            deserializer.descriptor == ByteArraySerializer().descriptor -> reader.nextObject() as T
        ...
        override fun decodeElementIndex(descriptor: SerialDescriptor): Int {
            while (true) {
                if (reader.currentId == -1) // EOF
                    return READ_DONE
                val index = getIndexByTag(descriptor, reader.currentId, false)
    ...
    private inner class MapEntryReader(decoder2: ProtobufReader, val parentTag: ProtoDesc?) :
        ProtobufDecoder(decoder2) {
        override fun SerialDescriptor.getTag(index: Int): ProtoDesc =
    ...
    override fun <T> load(deserializer: DeserializationStrategy<T>, bytes: ByteArray): T {
        ...
        val decoder = ProtobufDecoder(ProtobufReader(input))
